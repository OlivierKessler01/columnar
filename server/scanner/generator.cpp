 ...
 *         |           |
 *        ...         ... : those are inside layers of a and b, could be a
 *complex nfas |           | v           v (Accepting State of A) (Accepting
 *State of B) |           |
 *          \         /
 *           ε\     /ε
 *             v   v
...
/**
 * concat_construct - Generates an NFA concatenation construct (ab) given two
 * nfas
 *
 *   (Start State of A)
 *         |
...
/**
 * kleene_constuct - Generates an NFA Kleene's closure (a*) construct from an
 * nfa
 *
 * In place algorithm.
 *
...
struct state_set_pred {
  bool
  operator()(const std::unordered_set<state, state::hash_function> &s1,
